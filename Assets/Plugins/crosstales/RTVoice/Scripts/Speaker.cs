...
namespace Crosstales.RTVoice
{
   ...
   public class Speaker : Crosstales.Common.Util.Singleton<Speaker>
   {
      ...
      [FormerlySerializedAs("MaryTTSMode")] [Header("MaryTTS"), Tooltip("Enable or disable MaryTTS (default: false)."), SerializeField]
      private bool maryTTSMode;

      [FormerlySerializedAs("MaryTTSUrl")] [Tooltip("Server URL for MaryTTS."), SerializeField] private string maryTTSUrl = "http://mary.dfki.de";

      [FormerlySerializedAs("MaryTTSPort")] [Tooltip("Server port for MaryTTS (default: 59125)."), Range(0, 65535), SerializeField]
      private int maryTTSPort = 59125;

      [FormerlySerializedAs("MaryTTSUser")] [Tooltip("User name for MaryTTS (default: empty)."), SerializeField]
      private string maryTTSUser = string.Empty;

      [FormerlySerializedAs("MaryTTSPassword")] [Tooltip("User password for MaryTTS (default: empty)."), SerializeField]
      private string maryTTSPassword = string.Empty;

      [FormerlySerializedAs("MaryTTSType")] [Tooltip("Input type for MaryTTS (default: MaryTTSType.RAWMARYXML)."), SerializeField]
      private Model.Enum.MaryTTSType maryTTSType = Model.Enum.MaryTTSType.RAWMARYXML;
      ...
      //[FormerlySerializedAs("DontDestroy")] [Tooltip("Don't destroy gameobject during scene switches (default: true)."), SerializeField]
      ...
      */
      ...
      public int MaryTTSPort
      {
         ...
         set
         {
            if (maryTTSPort == value) return;

            maryTTSPort = Mathf.Clamp(value, 0, 65535);
      ...
      public string MaryTTSPassword
      {
         ...
         set
         {
            if (maryTTSPassword == value) return;

            maryTTSPassword = value;
      ...
      public Model.Enum.MaryTTSType MaryTTSType
      {
         get => maryTTSType;
         set => maryTTSType = value;
      }
...
*/
      ...
      protected override void Awake()
      {
         ...
         if (Instance == this)
         {
            ...
            if (Util.Helper.isLinuxPlatform && !MaryTTSMode)
               eSpeakMode = true;
      ...
      public float ApproximateSpeechLength(string text, float rate = 1f, float wordsPerMinute = 175f, float timeFactor = 0.9f)
      {
         ...
         if (Common.Util.BaseHelper.isWindowsPlatform && !MaryTTSMode && !ESpeakMode && !CustomMode)
         {
         ...
         if (voiceProvider != null)
         {
            ...
            else
            {
               ...
               else
               {
                  ...
                  if (!Util.Helper.isMacOSPlatform && !Util.Helper.isWSABasedPlatform && !MaryTTSMode) //prevent "double-speak"
                  {
                     wrapper.Volume = 0f;
      ...
      private void initProvider()
      {
         ...
         if (!useCustom)
         {
            ...
            if (MaryTTSMode)
            {
               if (MaryTTSUrl.Contains("mary.dfki.de") || MaryTTSUser.CTEquals("rtvdemo"))
               {
                  if (Util.Helper.isEditor)
                  {
                     Debug.LogWarning("You are using the test server of MaryTTS. Please request an account for our service at 'rtvoice@crosstales' or setup your own server from 'http://mary.dfki.de'.", this);

                     voiceProvider = mainVoiceProvider = Provider.VoiceProviderMary.Instance;
                  }
                  else
                  {
#if CT_DEVELOP
                     voiceProvider = mainVoiceProvider = Provider.VoiceProviderMary.Instance;
#else
                     Debug.LogError("You are using the test server of MaryTTS - this is not allowed in builds! Please request an account for our service at 'rtvoice@crosstales' or setup your own server from 'http://mary.dfki.de'.", this);
